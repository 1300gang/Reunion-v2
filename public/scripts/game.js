...
function displayGmSummary(summary) {
    ...
    // Affiche les graphiques thématiques
    displayThematicCharts(summary);
}
...
function createRadarChart(canvasId, labels, datasets) {
    ...
    // Détruit l'instance précédente du graphique si elle existe, pour éviter les bugs d'affichage
    if (Chart.getChart(canvasId)) {
        Chart.getChart(canvasId).destroy();
    }

    const primaryColor = gameConfig.levelColors ? gameConfig.levelColors[0] : '#e9bc40';
    const secondaryColor = gameConfig.levelColors ? gameConfig.levelColors[1] : '#c9e2e5';

    // Helper pour convertir hex en rgb pour les fonds transparents
    const hexToRgb = (hex) => {
        let r = 0, g = 0, b = 0;
        if (hex.length === 4) {
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: datasets.map((ds, index) => ({
                ...
                data: ds.data,
                fill: true,
                backgroundColor: index === 0 ? `rgba(${hexToRgb(primaryColor)}, 0.2)` : `rgba(${hexToRgb(secondaryColor)}, 0.2)`,
                borderColor: index === 0 ? primaryColor : secondaryColor,
                pointBackgroundColor: index === 0 ? primaryColor : secondaryColor,
                pointBorderColor: '#fff',
                pointHoverBackgroundColor: '#fff',
                pointHoverBorderColor: index === 0 ? primaryColor : secondaryColor
            }))
        },
        options: {
            responsive: true,
...
function displayThematicCharts(summary) {
    const { playerSummaries } = summary;
    const thematicAnalysisSection = document.querySelector('.thematic-analysis-section');

    if (!playerSummaries || playerSummaries.length === 0) {
        if (thematicAnalysisSection) thematicAnalysisSection.style.display = 'none';
        return;
    }
    ...
    playerSummaries.forEach(player => {
        if (player.thematicScores) {
            Object.keys(player.thematicScores).forEach(theme => {
                allThemes.add(theme);
                groupScores[theme] = (groupScores[theme] || 0) + player.thematicScores[theme];
            });
        }
    });

    const themeLabels = Array.from(allThemes);
    if (themeLabels.length < 3) {
        console.log("Pas assez de thèmes pour générer un graphique radar pertinent.");
        if (thematicAnalysisSection) thematicAnalysisSection.style.display = 'none';
        return;
    }

    if (thematicAnalysisSection) thematicAnalysisSection.style.display = 'block';

    const groupData = themeLabels.map(theme => groupScores[theme] || 0);
    createRadarChart('group-radar-chart', themeLabels, [{ label: 'Score du Groupe', data: groupData }]);

    const individualContainer = document.getElementById('individual-charts-container');
    individualContainer.innerHTML = '<h4>Profils Individuels</h4>';

    playerSummaries.forEach((player, index) => {
        const playerData = themeLabels.map(theme => (player.thematicScores && player.thematicScores[theme]) || 0);

        const chartWrapper = document.createElement('div');
        chartWrapper.className = 'chart-container individual';
        const canvasId = `player-chart-${index}`;
        chartWrapper.innerHTML = `<h5>${player.playerName}</h5><canvas id="${canvasId}"></canvas>`;
        individualContainer.appendChild(chartWrapper);

        createRadarChart(canvasId, themeLabels, [{ label: player.playerName, data: playerData }]);
    });
}
