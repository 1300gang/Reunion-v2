...
// La nouvelle fonction pour la transition d'interface A SUUP SI PAS BON!
function setupGameScreenForIntervenant() {
...
// Fonction startGame() mise à jour
function startGame() {
  console.log('🚀 Démarrage de la partie...');
  // Appel de la nouvelle fonction ici pour préparer l'écran
  setupGameScreenForIntervenant();
...
function displayIntervenantQuestion(questionData) {
  // Appeler la fonction pour la transition d'interface
  setupGameScreenForIntervenant();

  // Mettre à jour l'image contextuelle ici, pour TOUS les types de questions
  ...
  if (imageEl) {
    if (questionData.contextual_image) {
      ...
      showElement('questionImage'); // S'assurer que l'image est visible
    } else {
      hideElement('questionImage'); // Masquer l'image si elle n'est pas présente
    }
  }
...
function handleIntervenantContinue(questionData) {
  ...
  // Créer un bouton simple pour que le MJ puisse avancer
  const votingDiv = document.getElementById('votingOptions');
  if (votingDiv) {
    votingDiv.innerHTML = `
      <div class="continue-control">
        <h3>📖 Suite de l'histoire</h3>
        <p>${questionData.context || 'Les joueurs lisent la suite...'}</p>
        <button id="mjContinueBtn" class="button button-primary" style="margin-top: 20px;">
          Continuer pour tous les joueurs →
        </button>
        <div id="playerProgressList" style="margin-top: 15px;">
    ...
    const continueBtn = document.getElementById('mjContinueBtn');
    if (continueBtn && questionData.nextQuestions && questionData.nextQuestions.A) {
      continueBtn.onclick = () => {
        console.log('MJ force la continuation vers:', questionData.nextQuestions.A);
...
function handlePlayerContinue(questionData) {
  console.log(`⏱️ Début de l'affichage de la question Continue: ${questionData.id}`);
  ...
  continueBtn.onclick = () => {
    // Envoyer une réponse normale au serveur
    socket.emit('player-answer', { 
      questionId: questionData.id, 
      answer: 'A'  // Les questions Continue ont toujours la réponse A
    });
    
    console.log(`⏱️ Joueur a cliqué sur continuer pour ${questionData.id}`);
    
    continueBtn.classList.add('clicked');
    continueBtn.disabled = true;
    
    document.getElementById('answerStatus').textContent = '✅ En attente du maître de jeu...';
  ...
  console.log(`✅ Traitement de la question Continue terminé: ${questionData.id}`);
}
...
async function displayMessengerConversation(conversation) {
  ...
  if (gameState.currentQuestion && gameState.currentQuestion.isContinue) {
    console.log('💬 Conversation terminée, bouton Continuer disponible');
  }
}
...
function initializeSocketEvents() {
  ...
  socket.on('vote-update', (voteData) => {
    if (gameConfig.mode === 'intervenant') {
      ...
      // Pour les questions Continue, mettre à jour la liste de progression
      if (gameState.currentQuestion && gameState.currentQuestion.isContinue) {
        updateContinueProgress(voteData);
      }
...
function updateContinueProgress(voteData) {
  const progressList = document.getElementById('playerProgressList');
  if (!progressList) return;
  
  const { voteCounts, voteDetails } = voteData;
  const playersWhoVoted = voteDetails.A || [];
  
  if (playersWhoVoted.length > 0) {
    progressList.innerHTML = `
      <p style="color: var(--couleur-primary);">
        ${playersWhoVoted.length}/${gameState.allPlayers.length} joueurs ont continué
      </p>
      <div style="font-size: 0.9em; opacity: 0.8;">
        ${playersWhoVoted.join(', ')}
      </div>
    `;
  }
  
  // Si tous ont voté, activer automatiquement la continuation
  if (playersWhoVoted.length === gameState.allPlayers.length) {
    const mjContinueBtn = document.getElementById('mjContinueBtn');
    if (mjContinueBtn) {
      mjContinueBtn.style.background = 'var(--couleur-primary)';
      mjContinueBtn.textContent = '✅ Tous ont lu - Cliquez pour continuer';
    }
  }
}
...
function isContinueQuestion(questionData) {
  const isContinue = questionData && 
         questionData.choices && 
         questionData.choices.length === 1 && 
         (questionData.choices[0].toLowerCase() === 'continuer' || 
          questionData.choices[0].toLowerCase() === 'suite' ||
          questionData.question === '' ||
          questionData.question === null ||
          questionData.question === 'Continuer');
          
  if (isContinue) {
    console.log(`🔍 La question ${questionData.id} est bien une question "Continuer".`);
  }
